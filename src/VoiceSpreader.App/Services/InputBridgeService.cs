...
public sealed class InputBridgeService : IDisposable
{
    ...
    /// <summary>最近一次 SendInput 的 Win32 错误码；0 表示没有可用错误码。</summary>
    public int LastNativeError => Volatile.Read(ref _lastNativeError);

    private int _lastNativeError;
    ...
    /// <summary>使用 Ctrl+滚轮模拟应用通用的捏合缩放手势。</summary>
    public bool Zoom(double delta)
    {
        if (_disposed || Math.Abs(delta) < 0.001)
        {
            return true;
        }

        var wheel = ToWheelData(delta * 1000.0);
        var inputs = new[]
        {
            CreateKeyboardInput(0x11, 0),
            CreateMouseInput(0, 0, wheel, MouseWheel),
            CreateKeyboardInput(0x11, KeyboardKeyUp),
        };
        return TrySend(inputs) == (uint)inputs.Length;
    }
    ...
    private uint TrySend(INPUT[] inputs)
    {
        try
        {
            var sent = SendInput((uint)inputs.Length, inputs, Marshal.SizeOf<INPUT>());
            Volatile.Write(ref _lastNativeError, sent == 0 ? Marshal.GetLastWin32Error() : 0);
            return sent;
        }
        catch (DllNotFoundException)
        {
            // 非 Windows 测试环境或精简运行环境没有 user32 时，输入事件不能影响控制连接。
            Volatile.Write(ref _lastNativeError, 0);
            return 0;
        }
        ...
        catch (Exception)
        {
            ...
            Volatile.Write(ref _lastNativeError, Marshal.GetLastWin32Error());
            return 0;
        }
    }
